...
begin
  ...
  AgregarElemento(vector[1], 7);

  AgregarElemento(vector[2], 20);

  // Imprimir las listas
  writeln('Lista en la posición 1:');
  ImprimirLista(vector[1]);

  writeln('Lista en la posición 2:');
  ImprimirLista(vector[2]);
  //otra forma-----------------------
  for i:= 1 to 2 do begin
    while vector[i] <> nil do begin  
    writeln('------Lista en la posición :', i);
    writeln(vector[i]^.dato);
